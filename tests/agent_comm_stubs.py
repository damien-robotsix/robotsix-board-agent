...
"""Agent-comm stub classes and sys.modules injection.

Import this module (or ``import tests.agent_comm_stubs``) to inject
minimal ``robotsix_agent_comm`` stubs into ``sys.modules`` before any
test imports so that the agent module can import them without error.

Stub classes are importable directly::

    from tests.agent_comm_stubs import Request, Response, Agent
"""
